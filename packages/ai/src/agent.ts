...
export class AIAgent extends BaseActor<AgentMetadata> implements IIAgent {
  ...
  async generateObject<TObject = any>(
    ...
    const [llmConfig, { store, threadMessages }]: [LLMConfig, StorageResult] =
      await Promise.all([
        ...
        this._withTelemetry(
          ...
          async () =>
            await getLLMConfig({
              modelId: this._configuration?.model ?? DEFAULT_MODEL.id,
              llmVault: this.llmVault,
            }),
          timings,
          tracer,
  ...
  async generate(
    ...
    const [toolsets, agentOverrides, llmConfig, { store, threadMessages }] =
      await Promise.all([
    ...
    const result = await generateText({
      ...
      temperature: this._temperature(options?.temperature),
      maxOutputTokens: this._maxOutputTokens(options?.maxTokens),
      system: options?.instructions ?? agentOverrides.instructions,
    });
